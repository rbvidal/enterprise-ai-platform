...
C4Context
    ...
    Person(user, "Engineer", "Uploads documents, runs searches, asks AI questions")
    
    System(platform, "Enterprise AI Platform", "Java 21 / Spring Boot 3.3 modular monolith")
    
    System_Ext(postgres, "PostgreSQL", "Relational persistence — documents, users, chunks, audit")
    System_Ext(qdrant, "Qdrant", "Vector similarity search (optional)")
    System_Ext(neo4j, "Neo4j", "Knowledge graph persistence (optional)")
    System_Ext(ollama, "Ollama", "Local LLM inference and embeddings (optional)")
    System_Ext(openai, "OpenAI API", "Cloud LLM inference (optional)")

    Rel(user, platform, "Uses browser on :8080", "HTTPS")
    Rel(platform, postgres, "Reads/writes", "JDBC :5433")
    Rel(platform, qdrant, "Indexes/searches vectors", "REST :6333")
    Rel(platform, neo4j, "Persists/traverses graph", "Bolt :7687")
    Rel(platform, ollama, "Inference, embeddings", "REST :11434")
    Rel(platform, openai, "Inference", "REST HTTPS")
```
...
C4Container
    ...
    Container(web, "Web Application", "Spring Boot / Thymeleaf", "REST + server-rendered UI")
    Container(db, "PostgreSQL", "pgvector/pg16", "Documents, users, chunks, audit log")
    Container(qdrant_svc, "Qdrant", "Vector DB", "Embedding similarity search")
    Container(neo4j_svc, "Neo4j", "Graph DB", "Knowledge graph traversal")
    Container(llm, "LLM Server", "Ollama / OpenAI", "Inference and embeddings")

    Rel(web, db, "JDBC", "Persistent state")
    Rel(web, qdrant_svc, "REST API", "Vector indexing and search")
    Rel(web, neo4j_svc, "Bolt", "Graph persistence and traversal")
    Rel(web, llm, "HTTP REST", "Chat completion and embeddings")
```
...
graph TD
    subgraph "Application Layer"
        api[platform-api<br/>REST + Thymeleaf + Assembly]
    end
    
    subgraph "Platform Layer"
        ai[platform-ai<br/>RAG Orchestration]
        search[platform-search<br/>Hybrid Search]
        neo4j[platform-neo4j<br/>Knowledge Graph]
        doc[platform-document<br/>Document Lifecycle]
        ws[platform-workspace<br/>Workspace Wizard]
        obs[platform-observability<br/>Metrics + Health]
        auth[platform-auth<br/>JWT Authentication]
    end
    
    subgraph "Foundation Layer"
        audit[platform-audit<br/>Immutable Audit Log]
    end
...
```mermaid
C4Component
    title Component Diagram — AI Inference Path

    Container_Boundary(platform, "Enterprise AI Platform") {
        Component(controller, "AiPageController", "Thymeleaf", "Renders AI query form and results")
        Component(aiFacade, "AiFacade", "Java SPI", "Orchestrates full AI pipeline")
        Component(promptReg, "PromptRegistry", "Java SPI", "Versioned prompt templates")
        Component(providerRouter, "ProviderRouter", "Java SPI", "4-tier model selection")
        Component(retrievalOrch, "RetrievalOrchestrator", "Java SPI", "Intent → strategy → search")
        Component(evaluation, "EvaluationService", "Java SPI", "Quality assessment")
        
        Component(llmProvider, "ChatCompletionProvider", "Java SPI", "LLM backend")
        Component(searchFacade, "SearchFacade", "Java SPI", "Hybrid search entry point")
    }

    Rel(controller, aiFacade, "Calls answer()")
    Rel(aiFacade, retrievalOrch, "Orchestrates retrieval")
    Rel(aiFacade, promptReg, "Resolves prompt template")
    Rel(aiFacade, providerRouter, "Routes to provider")
    Rel(aiFacade, evaluation, "Evaluates quality")
    Rel(aiFacade, llmProvider, "Calls complete()")
    Rel(aiFacade, searchFacade, "Executes search")
```
...
sequenceDiagram
    ...
    participant Config as @ConfigurationProperties
    participant Conditional as @ConditionalOnProperty
    participant Registry as Registries
    participant Health as Health Indicators
    participant Tomcat as Embedded Tomcat

    Main->>Spring: run(PlatformApiApplication.class)
    Spring->>Scan: scanBasePackages("com.cognitera.platform")
    Scan-->>Spring: @Service, @Component, @Repository beans
    Spring->>Config: Bind application.yml to typed records
    Spring->>Conditional: Evaluate activation conditions
    Conditional-->>Spring: Activate/deactivate provider beans
    Spring->>Registry: Seed PromptRegistry, ModelCapabilityRegistry, WorkflowEngine
    Spring->>Health: Auto-discover HealthIndicator beans
    Spring->>Tomcat: Start on port 8080
    Tomcat-->>Main: Ready
```
...
sequenceDiagram
    ...
    participant Processor as IngestionProcessor
    participant Extraction as TextExtractionService
    participant Enrichment as EnrichmentHook
    participant Chunking as ChunkingStrategy
    participant Embedding as EmbeddingProvider
    participant PostgreSQL
    participant Qdrant
    participant Neo4j

    User->>Controller: POST /api/documents (multipart)
    Controller->>Service: createDocument(file)
    Service->>PostgreSQL: INSERT document + ingestion_job (PENDING)
    Service-->>Controller: Document created

    Note over Worker: @Scheduled every 10s
    Worker->>PostgreSQL: SELECT pending jobs (LIMIT 10)
    loop For each pending job
        Worker->>Processor: ingest(documentId)
        Processor->>Extraction: extractText(type, version)
        Extraction-->>Processor: Raw text
        
        Processor->>Enrichment: enrich(documentId, text)
        Enrichment->>Neo4j: Persist entities, concepts, relationships
        
        Processor->>Chunking: chunk(text)
        Chunking-->>Processor: List<DocumentChunk>
        
        Processor->>Embedding: embedBatch(texts)
        Embedding-->>Processor: List<float[]>
        
        Processor->>PostgreSQL: INSERT chunks (JPA)
        Processor->>Qdrant: PUT points with vectors
        Processor->>PostgreSQL: UPDATE ingestion_job (COMPLETED)
    end
```
...
sequenceDiagram
    actor User
    participant Controller as AiPageController
    participant Facade as AiFacade
    participant Service as AiService
    participant Intent as QueryIntentClassifier
    participant Retrieval as RetrievalAugmentationService
    participant Search as SearchFacade
    participant Prompt as PromptRegistry
    participant Router as ProviderRouter
    participant LLM as ChatCompletionProvider
    participant Validation as ClaimValidator
    participant Grounding as GroundingService
    participant Eval as EvaluationService

    User->>Controller: POST /ai (question)
    Controller->>Facade: answer(AiRequest)
    Facade->>Service: answer()
    
    Service->>Intent: classify(question)
    Intent-->>Service: QUERY_ANSWER / INDEX_INSPECTION
    
    Service->>Retrieval: retrieve(request)
    Retrieval->>Search: search(query, HYBRID)
    Search-->>Retrieval: SearchResultPage
    Retrieval-->>Service: RetrievalContext
    
    Service->>Prompt: getLatest("rag-answer")
    Prompt-->>Service: PromptTemplate (v1, RETRIEVAL)
    Service->>Service: Build and render prompt
    
    Service->>Router: routeChat(request)
    Router-->>Service: ChatCompletionProvider
    Service->>LLM: complete(prompt, capabilities)
    LLM-->>Service: Raw answer text
    
    Service->>Validation: validate(question, answer)
    Validation-->>Service: ValidationResult
    
    Service->>Grounding: ground(answer, sources)
    Grounding-->>Service: ReasonedAnswer
    
    Service->>Eval: evaluate(question, answer, context)
    Eval-->>Service: EvaluationResult
    
    Service-->>Controller: AiResponse(answer, metadata)
    Controller-->>User: Rendered HTML with answer + sources
```
...
classDiagram
    class ChatCompletionProvider {
        ...
        +providerName() String
        +isAvailable() boolean
        +complete(String prompt, ModelCapabilities) String
    }
    ...
    class ProviderRouter {
        <<interface>>
        +routeChat(InferenceRequest) ChatCompletionProvider
        +resolveProviders(CapabilityRequest) List
    }
    
    class DefaultProviderRouter {
        -List~ChatCompletionProvider~ providers
        +routeChat() ChatCompletionProvider
    }
    
    ChatCompletionProvider <|.. OllamaChatProvider
    ChatCompletionProvider <|.. OpenAiChatProvider
    ProviderRouter <|.. DefaultProviderRouter
    DefaultProviderRouter --> ChatCompletionProvider : selects from
```
...
```mermaid
flowchart LR
    A[Document Text] --> B{LLM Available?}
    B -->|Yes| C[LLM Extraction<br/>entity-extraction/v1 prompt]
    B -->|No| D[Regex Extraction<br/>ORGANIZATION, PERSON, DATE, MONEY]
    C --> E[EnrichmentContext]
    D --> E
    E --> F[EnrichmentHook]
    F --> G{Neo4j Available?}
    G -->|Yes| H[GraphEnrichmentService.persist]
    G -->|No| I[Skip — enrichment result discarded]
    H --> J[Neo4j Graph<br/>with NodeProvenance]
```
...
flowchart TB
    subgraph Retrieval["Retrieval Pipeline"]
        Q[User Query]
        KW[Keyword Search<br/>JPA/TF scoring]
        VEC[Vector Search<br/>Qdrant cosine]
        GRAPH[Graph Search<br/>Neo4j traversal]
        FUSION[Weighted Linear Fusion]
    ...
    GRAPH -.->|optional| NEO4J[Neo4jGraphSearchAdapter]
    NEO4J -.->|bridges to| GES[GraphEnrichmentService]
    
    style GRAPH fill:#f9f,stroke:#333,stroke-dasharray: 5 5
    style NEO4J fill:#f9f,stroke:#333,stroke-dasharray: 5 5
```
...
sequenceDiagram
    ...
    participant Hybrid as DefaultHybridRetrievalService
    participant Keyword as KeywordSearchProvider
    participant Vector as VectorSearchProvider
    participant Graph as GraphSearchProvider
    participant Neo4j as Neo4jGraphSearchAdapter

    Orchestrator->>Hybrid: retrieve(query)
    
    par Parallel Retrieval
        Hybrid->>Keyword: search(query)
        Keyword-->>Hybrid: keywordResults (10-20)
        Hybrid->>Vector: search(query)
        Vector-->>Hybrid: vectorResults (10-20)
        Hybrid->>Graph: search(query)
        Graph->>Graph: isAvailable()?
        alt Neo4j Available
            Graph->>Neo4j: traverse(seedIds, 2)
            Neo4j-->>Graph: List<GraphNode>
            Graph-->>Hybrid: graphResults (3-8)
        else Neo4j Unavailable
            Graph-->>Hybrid: empty list
        end
    end
    
    Hybrid->>Hybrid: merge(keyword, vector, graph)
    Note over Hybrid: keyword×0.40 + vector×0.40<br/>+ confidence×0.20<br/>+ graphBoost (up to 0.15)
    Hybrid-->>Orchestrator: Fused candidates
```
...
```mermaid
flowchart TD
    Q[User Query] --> IC[IntentClassifier.classify]
    IC -->|INDEX_INSPECTION| KW[KEYWORD Strategy]
    IC -->|CORPUS_DISCOVERY| KW
    IC -->|QUESTION_ANSWERING| HY[HYBRID Strategy]
    IC -->|WORKSPACE_ANALYSIS| HY
    IC -->|SOURCE_ANALYSIS| HY
    IC -->|DOCUMENT_RESEARCH| SEM[SEMANTIC Strategy]
    IC -->|DOCUMENT_LOOKUP| SEM
    
    KW --> KWS[KeywordSearchProvider only]
    HY --> HYS[Keyword + Vector + Graph]
    SEM --> SEMS[VectorSearchProvider only]
    
    KWS --> FUSION[Weighted Linear Fusion]
    HYS --> FUSION
    SEMS --> FUSION
    FUSION --> RERANK[Two-Stage Reranking]
    RERANK --> RESULT[Ranked Candidates]
```
...
flowchart LR
    subgraph Inputs
        K[Keyword Results<br/>TF score: 0.0-1.0]
        V[Vector Results<br/>Cosine score: 0.0-1.0]
        G[Graph Results<br/>Traversal score: 0.0-1.0]
    end
    
    subgraph Merge["Merge by Chunk ID (LinkedHashMap dedup)"]
        M[For each unique chunk:]
    end
    
    subgraph Scoring["Score Calculation"]
        S["rankingScore = (keyword×0.40 + vector×0.40 + confidence×0.20)<br/>× docTypeWeight<br/>+ graphBoost (max 0.15)<br/>clamped to [0, 1]"]
    end
...
classDiagram
    ...
    class PromptRegistry {
        <<interface>>
        +get(String qualifiedId) Optional
        +getLatest(String promptId) Optional
        +listPromptIds() List
        +getVersions(String promptId) List
        +findByCategory(Category) List
        +register(PromptTemplate) void
    }
...
classDiagram
    class ModelCapability {
        ...
        +List~String~ tags
        +supports(CapabilityRequest) boolean
    }
    ...
    class ModelCapabilityRegistry {
        <<interface>>
        +get(String modelName) Optional
        +findByProvider(String) List
        +findByCapability(CapabilityRequest) List
        +findByProviderAndCapability(String, CapabilityRequest) List
        +listAll() List
    }
    
    ModelCapability --> CapabilityRequest
    ModelCapabilityRegistry --> ModelCapability
```
...
```mermaid
flowchart TD
    REQ[InferenceRequest] --> T1{Model prefix?<br/>e.g., openai:gpt-4o}
    T1 -->|Yes| P1[Route to matching provider]
    T1 -->|No| T2{Capability requested?<br/>e.g., STREAMING}
    T2 -->|Yes| P2[Query ModelCapabilityRegistry<br/>→ route to capable provider]
    T2 -->|No| T3{Preferred provider set?}
    T3 -->|Yes, available| P3[Route to preferred provider]
    T3 -->|No or unavailable| T4{Any provider available?}
    T4 -->|Yes| P4[First available provider]
    T4 -->|No| ERR[throw IllegalStateException]
    
    P1 --> DONE[Return ChatCompletionProvider]
    P2 --> DONE
    P3 --> DONE
    P4 --> DONE
```
...
```mermaid
stateDiagram-v2
    [*] --> SETUP: start("document-intelligence")
    SETUP --> INGESTION: advance()
    INGESTION --> ANALYSIS: advance()
    ANALYSIS --> REVIEW: advance()
    REVIEW --> COMPLETE: advance()
    COMPLETE --> COMPLETE: advance() → COMPLETED status
    
    INGESTION --> SETUP: previous()
    ANALYSIS --> INGESTION: previous()
    REVIEW --> ANALYSIS: previous()
    COMPLETE --> REVIEW: previous()
    
    state SETUP {
        [*] --> SetupActive
    }
    state COMPLETE {
        CompleteActive --> [*]: advance()
    }
```
...
flowchart TB
    subgraph Platform["Enterprise AI Platform"]
        AI[AI Operations<br/>Inference, Embedding, Retrieval]
        ING[Document Operations<br/>Ingestion, Enrichment]
        SYS[System Operations<br/>Health, Availability]
    end
    
    subgraph Metrics["AiMetrics"]
        INF_TIMER[ai.inference.duration<br/>Timer + Histogram]
        EMB_TIMER[ai.embedding.duration<br/>Timer]
        RET_TIMER[ai.retrieval.duration<br/>Timer]
        GRA_TIMER[ai.graph.retrieval.duration<br/>Timer]
        ENR_TIMER[ai.enrichment.duration<br/>Timer]
        PRO_GAUGE[ai.provider.available<br/>Gauge 0/1]
        EVAL_SUM[ai.evaluation.*<br/>Summary]
    end
    ...
    HEALTH --> ALERTING[Alerting<br/>Rules]
```
...
```mermaid
graph TB
    subgraph "Playwright (UI) — 12 tests"
        PW["mvn verify -Pui-tests<br/>Browser user journeys"]
    end
    subgraph "Architecture + Workflow — 19 tests"
        ARCH["mvn verify<br/>End-to-end behavioral validation"]
    end
    subgraph "Integration — 91 tests"
        INT["mvn verify<br/>Subsystem interaction with Spring context"]
    end
    subgraph "Unit — 44 tests"
        UNIT["mvn test<br/>Isolated component behavior"]
    end
...
```mermaid
C4Deployment
    title Deployment — Enterprise AI Platform

    Deployment_Node(dev, "Development Machine", "Java 21, Maven") {
        Container(app, "Spring Boot Application", "platform-api", "Port 8080")
    }
    
    Deployment_Node(db_node, "PostgreSQL Server", "Docker") {
        Container(db, "PostgreSQL", "pgvector/pg16", "Port 5433")
    }
    
    Deployment_Node(qdrant_node, "Qdrant Server", "Docker") {
        Container(qdrant, "Qdrant", "latest", "Ports 6333/6334")
    }
    
    Deployment_Node(neo4j_node, "Neo4j Server", "Docker — graph profile") {
        Container(neo4j, "Neo4j", "5-community", "Ports 7474/7687")
    }
    
    Deployment_Node(llm_node, "LLM Server", "Optional") {
        Container(ollama, "Ollama", "latest", "Port 11434")
    }

    Rel(app, db, "JDBC", "TCP :5433")
    Rel(app, qdrant, "REST", "TCP :6333")
    Rel(app, neo4j, "Bolt", "TCP :7687")
    Rel(app, ollama, "REST", "TCP :11434")
```
